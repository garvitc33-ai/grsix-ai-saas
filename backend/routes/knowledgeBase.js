...
router.post("/upload", upload.single("file"), async (req, res) => {
  ...
  try {
    ...
    await saveKnowledgeBase({ name: companyName, source_type: "manual", content });
    fs.unlinkSync(file.path); // cleanup
    res.status(200).json({ message: "✅ Knowledge base uploaded successfully" });
  } catch (err) {
    if (file?.path && fs.existsSync(file.path)) fs.unlinkSync(file.path); // ensure cleanup
    res.status(500).json({ error: "Failed to save knowledge base" });
  }
});

// === 2️⃣ Generate KB via Website Scraping ===
router.post("/generate", async (req, res) => {
  ...
  try {
    ...
    await saveKnowledgeBase({ name: companyName, source_type: "website", content: scrapedText });
...
router.get("/:companyName", async (req, res) => {
  ...
  try {
    const data = await getKnowledgeBaseByName(name);
...
router.post("/:companyName", async (req, res) => {
  ...
  try {
    await updateKnowledgeBaseByName(name, content);
...
// === 7️⃣ Save AI-Generated KB (no file upload) ===
router.post("/save-ai", async (req, res) => {
  ...
  try {
    await saveKnowledgeBase({ name: companyName, source_type: "website", content });
    res.status(200).json({ message: "✅ AI KB saved successfully" });
  } catch (err) {
